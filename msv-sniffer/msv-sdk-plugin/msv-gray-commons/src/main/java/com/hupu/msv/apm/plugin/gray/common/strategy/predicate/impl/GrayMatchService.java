...
public enum GrayMatchService {
    ...
    public GrayRule matchedServiceList(Object args) {
        ...
        if (!GrayRouteManager.INSTANCE.getGrayCommonConfig().getExtensionHttpHeaders().isEmpty()
            ...
            GrayMatchService.MapEntry<String, GrayRule> pathPredicate = matchedServiceListByPath(path);
    ...
    private MapEntry<String, GrayRule> matchedServiceListByPath(String path) {
        if (GrayRouteManager.INSTANCE.getGrayRouteStrategy().getStrictPathPredicate().containsKey(path)) {
            return new MapEntry(path, GrayRouteManager.INSTANCE.getGrayRouteStrategy().getStrictPathPredicate().get(path));
        }

        for (String pattern : GrayRouteManager.INSTANCE.getGrayRouteStrategy().getFuzzyPathPredicate().keySet()) {
            if (GrayMatchService.pathMatcher.match(pattern, path)) {
                return new MapEntry(pattern, GrayRouteManager.INSTANCE.getGrayRouteStrategy().getFuzzyPathPredicate().get(pattern));
            }
        }
