...
package com.hupu.msv.apm.agent.core.plugin.loader;
...
public class AgentClassLoader extends ClassLoader {
     ...
     */
    private static ClassLoader DEFAULT_LOADER;
    ...
    /**
     * 解决类装入器在jvm启动时死锁的问题(只支持JDK7+，因为ParallelCapable出现在JDK7+中)
     */
    ...
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        ...
        for (Jar jar : allJars) {
            ...
            if (entry != null) {
                try {
                    String spec = "jar:file:" + jar.sourceFile.getAbsolutePath() + "!/" + path;
                    logger.info(spec);
                    URL classFileUrl = new URL(spec);
                    byte[] data = null;
