/*
 * Licensed to the Apache Software Foundation (ASF) under one or more
 * contributor license agreements.  See the NOTICE file distributed with
 * this work for additional information regarding copyright ownership.
 * The ASF licenses this file to You under the Apache License, Version 2.0
 * (the "License"); you may not use this file except in compliance with
 * the License.  You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 */

package com.hupu.msv.apm.agent.core.context;

import java.util.*;

import com.hupu.msv.apm.agent.core.conf.Config;
import com.hupu.msv.apm.agent.core.conf.RemoteDownstreamConfig;
import com.hupu.msv.apm.agent.core.context.ids.DistributedTraceId;
import com.hupu.msv.apm.agent.core.context.ids.ID;
import com.hupu.msv.apm.agent.core.context.ids.NewDistributedTraceId;
import com.hupu.msv.apm.agent.core.context.ids.PropagatedTraceId;
import com.hupu.msv.apm.agent.core.context.trace.*;

/**
 * The <code>IgnoredTracerContext</code> represent a context should be ignored. So it just maintains the stack with an
 * integer depth field.
 * <p>
 * All operations through this will be ignored, and keep the memory and gc cost as low as possible.
 *
 * @author wusheng
 */
public class IgnoredTracerContext implements AbstractTracerContext {
    private static final NoopSpan NOOP_SPAN = new NoopSpan();

    private int stackDepth;

    private DistributedTraceId distributedTraceId;

    public IgnoredTracerContext() {
        this.stackDepth = 0;
        String traceId = ContextManager.getGlobalTraceId();
        if ("N/A".equals(traceId)) {
            distributedTraceId = new NewDistributedTraceId();
        } else {
            distributedTraceId = new PropagatedTraceId(traceId);
        }
    }

    public IgnoredTracerContext(String traceId) {
        this.stackDepth = 0;
        distributedTraceId = new PropagatedTraceId(traceId);
    }

    @Override
    public void inject(ContextCarrier carrier) {
        carrier.setDistributedTraceIds(Collections.singletonList(distributedTraceId));
        carrier.setSpanId(0);
        carrier.setParentServiceInstanceId(RemoteDownstreamConfig.Agent.SERVICE_INSTANCE_ID);
        carrier.setEntryServiceInstanceId(RemoteDownstreamConfig.Agent.SERVICE_INSTANCE_ID);
        carrier.setPeerHost(" ");
        carrier.setTraceSegmentId(new ID(distributedTraceId.toString()));
        carrier.setEntryEndpointName(" ");
        carrier.setParentEndpointName(" ");
    }

    @Override
    public void extract(ContextCarrier carrier) {
        this.distributedTraceId = carrier.getDistributedTraceId();
    }

    @Override
    public ContextSnapshot capture() {
        //return traceSegmentId != null
        //            && spanId > -1
        //            && entryApplicationInstanceId != DictionaryUtil.nullValue()
        //            && primaryDistributedTraceId != null;

        ContextSnapshot contextSnapshot = new ContextSnapshot(new ID(distributedTraceId.toString()),
                0, Collections.singletonList(distributedTraceId));
        contextSnapshot.setEntryApplicationInstanceId(RemoteDownstreamConfig.Agent.SERVICE_INSTANCE_ID);
        return contextSnapshot;
    }

    @Override
    public void continued(ContextSnapshot snapshot) {
        this.distributedTraceId = snapshot.getDistributedTraceId();
    }

    @Override
    public String getReadableGlobalTraceId() {
        return distributedTraceId == null ? null : distributedTraceId.toString();
    }

    @Override
    public AbstractSpan createEntrySpan(String operationName) {
        stackDepth++;
        return NOOP_SPAN;
    }

    @Override
    public AbstractSpan createLocalSpan(String operationName) {
        stackDepth++;
        return NOOP_SPAN;
    }

    @Override
    public AbstractSpan createExitSpan(String operationName, String remotePeer) {
        stackDepth++;
        return NOOP_SPAN;
    }

    @Override
    public AbstractSpan activeSpan() {
        return NOOP_SPAN;
    }

    @Override
    public boolean stopSpan(AbstractSpan span) {
        stackDepth--;
        if (stackDepth == 0) {
            distributedTraceId = null;
            ListenerManager.notifyFinish(this);
        }
        return stackDepth == 0;
    }

    @Override
    public AbstractTracerContext awaitFinishAsync() {
        return this;
    }

    @Override
    public void asyncStop(AsyncSpan span) {

    }

    public static class ListenerManager {
        private static List<IgnoreTracerContextListener> LISTENERS = new LinkedList<IgnoreTracerContextListener>();

        /**
         * Add the given {@link IgnoreTracerContextListener} to {@link #LISTENERS} list.
         *
         * @param listener the new listener.
         */
        public static synchronized void add(IgnoreTracerContextListener listener) {
            LISTENERS.add(listener);
        }

        /**
         * Notify the {@link IgnoredTracerContext.ListenerManager} about the given {@link IgnoredTracerContext} have
         * finished. And trigger {@link IgnoredTracerContext.ListenerManager} to notify all {@link #LISTENERS} 's {@link
         * IgnoreTracerContextListener#afterFinished(IgnoredTracerContext)}
         *
         * @param ignoredTracerContext
         */
        static void notifyFinish(IgnoredTracerContext ignoredTracerContext) {
            for (IgnoreTracerContextListener listener : LISTENERS) {
                listener.afterFinished(ignoredTracerContext);
            }
        }

        /**
         * Clear the given {@link IgnoreTracerContextListener}
         */
        public static synchronized void remove(IgnoreTracerContextListener listener) {
            LISTENERS.remove(listener);
        }
    }
}
